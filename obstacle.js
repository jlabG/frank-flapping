class Obstacle {
    constructor(game, x) {
        ...
        this.speedY = Math.random() < 0.5 ? -1 * this.game.ratio : 1 *
            this.game.ratio;
    ...
    update() {
        ...
        if (!this.game.gameOver) {
            if (this.y <= 0 || this.y >= this.game.height - this.
                scaledHeight) {
                this.speedY *= -1;
            }
        } else {
    ...
    draw() {
        this.game.ctx.drawImage(this.image, this.frameX * this.spriteWidth, 0,
            this.spriteWidth, this.spriteHeight, this.x, this.y, this.scaledWidth,
            this.scaledHeight)
        if (this.game.debug) {
